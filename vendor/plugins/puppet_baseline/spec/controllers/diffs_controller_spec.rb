...
describe DiffsController do
  describe "#diff" do
    it "should use the baseline of the node associated with the report if baseline_type=self" do
      ...
      Baseline.report_make_baseline!(baseline)
  ...
  describe "when diffing latest inspect report against baseline and" do
    ...
    describe "the requested baseline can't be found" do
      it "should raise an error if the specified node doesn't have a baseline" do
        ...
        lambda { get :diff_group, :id => @node_group.id, :baseline_type => "other", :baseline_host => "foo" }.should raise_error(ActiveRecord::RecordNotFound)
      end

      it "should raise an error if the node doesn't exist" do
        lambda { get :diff_group, :id => @node_group.id, :baseline_type => "other", :baseline_host => "non_existent_node" }.should raise_error(ActiveRecord::RecordNotFound)
      end
      ...
      it "should not raise an error if comparing against nodes' own baselines" do
        ...
        get :diff_group, :id => @node_group.id, :baseline_type => "self"
    ...
    describe "when diffing against a single baseline" do
      it "should diff each node against the given baseline" do
        ...
        Baseline.report_make_baseline! @baseline
        Baseline.report_make_baseline! @child_baseline
        @node.last_inspect_report = @latest
        @child_node.last_inspect_report = @child_latest

        get :diff_group, :id => @node_group.id, :baseline_type => "other", :baseline_host => @node.name
